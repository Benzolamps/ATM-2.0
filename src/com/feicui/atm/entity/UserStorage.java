...
public class UserStorage {
    ...
    public AtmUser getUserByAccount(String account) { // 通过账号查找user
    ...
    public AtmUser getUserByIdNumber(String idNumber) { // 通过身份证号查找user
    ...
    private static TreeSet<AtmUser> readUsers(){ // 读用户
        ...
        if (!file.exists()) { // 文件未创建, 初始化文件
            ...
            try {
                ...
                file.delete();
                UserStorage.writeUsers(new TreeSet<AtmUser>()); // 文件内容错误, 初始化文件
            } catch (IOException e) {
                CommonUtil.printStackTrace(e);
                file.delete();
                UserStorage.writeUsers(new TreeSet<AtmUser>()); // 文件内容错误, 初始化文件
    ...
    private static void writeUsers(TreeSet<AtmUser> users){ // 写用户
        if (users == null) {
            return;
        }
        File file = new File(UserStorage.FILE_NAME);
